...
struct WaveformView: View {
    ...
    // 23 discrete vertical bars. Each bar is a single solid little pillar —
    // a rounded rectangle expanding symmetrically from the center line — not
    // a stack of pixel cells. Bars stay separated with visible spacing so the
    // row still reads as discrete columns rather than one continuous shape.
    private let barCount = 23
    private let barWidth: CGFloat = 9
    private let barSpacing: CGFloat = 6
    // Small corner radius softens each pillar without making it a blob. Half
    // the bar width gives a gently rounded cap; capped at 2pt.
    private let barCornerRadius: CGFloat = min(2, 9 / 2)
        ...
        TimelineView(.animation(minimumInterval: 1.0 / 30.0, paused: mode == .idle)) { timeline in
            ...
            Canvas { context, size in
                ...
                for i in 0..<barCount {
                    ...
                    // A single solid pillar, vertically centered on the middle
                    // line so it grows symmetrically up and down.
                    ...
                    let radius = min(barCornerRadius, barWidth / 2, barHeight / 2)
