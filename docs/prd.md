...
## 工程变更（2026-05-28）：VoiceFlowKit 抽取

V1 的录音/转写底层 pipeline 从 2026-05-28 起抽到独立 SPM package `VoiceFlowKit`。VoiceFlow app 通过本地 SPM 依赖引用 Kit。这次抽取对**产品行为零影响**——上面所有 V0 / V1 交付状态保持不变，UI 不变、UX 不变、wire protocol 不变。

这次抽取的产品意义在于：未来 OpenCode iOS Client 等其他客户可以通过远程 SPM 依赖（GitHub repo `grapeot/voiceflow`）拿同一份 pipeline，避免两边各自实现各自修 bug。架构细节见 `docs/rfc.md` 的"模块划分"章节。
