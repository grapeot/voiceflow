...
- Stop 只 finalize（flush + `commit` + `stop`），文本随 `transcript_delta` 增量显示。
- 断线/ send 失败：磁盘 cache 重连并重放全部已录 PCM（不对齐时间轴 sleep）；partial transcript 保留；`TranscriptEpochMerger` 保证重连后 `isNewResponse` 不 wipe 断线前文本。
...
用户按下 Start 后，app 通过 Bearer token 创建 realtime session 并建立 ticket WebSocket，边录边以固定时长分片（约 0.5s、24 kHz PCM）发送音频。用户说话时，转写区可以尽早出现 `transcript_delta`。用户按下 Stop 时，flush 剩余 buffer 并发送 `commit` + `stop`，等待 `transcript_completed` / `session_stopped`。
...
- WebSocket 断开：保留 partial transcript；录音过程中不弹 modal，状态灯变红/橙，caption 显示「正在重连」或「流已断开」。
- 重连后 `isNewResponse` 不 wipe 断线前文本：`transcriptSnapshot` + `streamEpoch` 只在当前 epoch 内 append/replace。
