...
- 断线/ send 失败：磁盘 cache 重连并重放全部已录 PCM（不对齐时间轴 sleep）；partial transcript 保留。
- 重发录音：bulk WebSocket，无实时重放。
- 状态灯反映 stream 连接（绿/橙/红）；剪贴板 stream throttle。
- 单元测试 46 项（含 `RealtimeTranscriptionTests`）。
