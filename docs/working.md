...
- **Token 设置重做**：token 输入框始终存在，保存态以掩码 placeholder 表示，并允许直接输入新 token 覆盖旧值；输入框右侧 eye button 可显示/隐藏当前尚未保存的输入，便于检查引号、`Bearer ` 前缀或复制截断，但不会读取 Keychain 中的旧 token。删除独立 Save 步骤；输入新值时主按钮为 Save & Test，输入为空且已有 token 时直接测试已保存值。连接测试从不兼容新 Builder key 的 usage summary 改为实际受保护的 realtime-session ticket endpoint；不上传音频、不打开 WebSocket。失败不再吞掉 HTTP 状态，401/403/429/5xx 分别给出可行动提示。
