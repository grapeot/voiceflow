...
In Xcode: File → Add Package Dependencies → URL `https://github.com/grapeot/voiceflow.git`. Pin an exact SemVer release — currently **`0.4.0`** (`.package(url: "https://github.com/grapeot/voiceflow.git", exact: "0.4.0")`). Add the `VoiceFlowKit` product to your app target. Host apps that need reproducible builds should commit `Package.resolved`.
